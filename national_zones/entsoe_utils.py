...
class EntsoeData:
    ...
    def get_generation_data(self, zone_name=None, austria=False) -> pd.DataFrame:
        ...
        if not austria:
            ...
            for col in df_generation.columns:
                if col in col_count:
                    # If the column name has been seen before, append the count as a suffix
            ...
            df_generation.columns = new_columns
            ...
            date_index = pd.date_range(start=self.start, periods=len(df_generation), freq=self.range_time_generation)
            df_generation.index = date_index
            df_generation = df_generation.iloc[:, [0, 2, 4, 6, 17, 8, 12, 18, 20, 22, 24, 10, 14]]
            # print(df_generation.columns)
...
def calculate_emission_factor(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'Fossil Coal-derived gas' in df_generation.columns:
        df_generation['co2_fossilcoal'] = df_generation['Fossil Coal-derived gas'] * fossilgas_emission
    ...
    if 'Fossil hard coal' in df_generation.columns:
        df_generation['co2_fossilcoal'] = df_generation['Fossil hard coal'] * other_emission

    # sum the emissions
    emission_cols = ['co2_other', 'co2_fossilcoal', 'co2_fossilgas', 'co2_fossiloil']
