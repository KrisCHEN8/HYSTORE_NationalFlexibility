...
# Input data
D_H = df_demand['Austria_heating_kWh'].values * 1e-6  # be consistant with the country in line 7  # noqa: E501
D_C = df_demand['Austria_cooling_kWh'].values * 1e-6  # be consistant with the country in line 7  # noqa: E501
...
Cm_dict_ave = {
    'Cm_h': 157664.92 * 1e-6,   # GWh
    'Cm_c': 0 * 1e-6    # GWh
}

Cm_dict_70p = {
    'Cm_h': 1194809.98 * 1e-6,   # GWh
    'Cm_c': 0 * 1e-6   # GWh
}

Cm_dict_50p = {
    'Cm_h': 182477.33 * 1e-6,   # GWh
    'Cm_c': 0 * 1e-6   # GWh
}
...
if solver == 'Pyomo':
    ...
    # Cm_ave
    optimizer = PredictiveOptimizerCVXPY(D_H, D_C, df_agg, 12, COP_df['SE'], EER_df['SE'], Cm_dict_ave, 'surplus_RES')  # noqa: E501
    df_results = optimizer.opt(time_series[0], time_series[-1])
    df_results.index = time_series
    df_results.to_excel('./res/SE/results_SE_aveCm.xlsx', index=True)

    # Cm_70%
    optimizer = PredictiveOptimizerCVXPY(D_H, D_C, df_agg, 12, COP_df['SE'], EER_df['SE'], Cm_dict_70p, 'surplus_RES')  # noqa: E501
    df_results = optimizer.opt(time_series[0], time_series[-1])
    df_results.index = time_series
    df_results.to_excel('./res/SE/results_SE_70PCm.xlsx', index=True)

    # Cm_50%
    optimizer = PredictiveOptimizerCVXPY(D_H, D_C, df_agg, 12, COP_df['SE'], EER_df['SE'], Cm_dict_50p, 'surplus_RES')  # noqa: E501
    df_results = optimizer.opt(time_series[0], time_series[-1])
    df_results.index = time_series
    df_results.to_excel('./res/SE/results_SE_50PCm.xlsx', index=True)
